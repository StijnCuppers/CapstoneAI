...
input_folder = R"C:\Users\bramb\OneDrive\Documenten\MST-jaar 3\Minor Engineering with AI\Capstone\Capstone data (1)\Capstone data\Data"
...
def labelmatch(labels, labels_name, mode):
    """
    Matches a list of labels to their corresponding bubbles

    Args:
        labels (lst): list of labels.
        labels_name (str): name of which labels are used. Necesairy for output file.
        mode (str): Either 'seperate' or 'whole'

    Returns:
        Pandas dataframe containing either two columns or a single column of voltage data matched with the corresponding velocity.

    """
    # Selecting mode
    if mode == 'seperate':
        bubbles = pd.read_csv('2024-11-12T145426_seperate.csv')
    elif mode == 'whole':
        bubbles = pd.read_csv('2024-11-12T145426_whole.csv')
    
    # Initializing empty list for dataframes
    labeled_bubbles = []
    ...
    for i in range(len(labels)):
        ...
        #Chosing a final velocity to give
        if len(labels[i]) == 3:
            velocity = labels[i][2]
        else:
            velocity = min(labels[i][2],labels[i][3])

        #Creating checking values for the data dataframe
        arrival_time = (bubbles['tA0']<=t_a) & (t_a <= bubbles['tA1'])
        exit_time = (bubbles['tE0']<=t_e) & (t_e <= bubbles['tE1'])
        row_match = arrival_time & exit_time

        #Creating a new dataframe of voltages with a label
        if mode == 'seperate':
            bubble = bubbles.loc[row_match, 'voltage_entry': 'voltage_exit']
        else:
            bubble = bubbles.loc[row_match, 'voltage_full']
        bubble['Velocity'] = velocity

        labeled_bubbles.append(bubble)

    #The final dataframe
    bubbles_labeled = pd.concat(labeled_bubbles, ignore_index = False)

    #Creating an output file
    output_file = f"labeled bubbles of {labels_name}_{mode}.csv"
    bubbles_labeled.to_csv(output_file, index=False)
    print(f"Labeled bubbles data saves to {output_file}")

    return bubbles_labeled
"--------------------------------------------------------------------------------------------------------"
